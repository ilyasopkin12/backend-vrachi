...
const DOCTORS = [
  ['Иван', 'Петров', 'Терапевт', 'Москва', 14, 'Клиника «Здоровье», каб. 101', '101', 'OFFLINE'],
  ['Мария', 'Сидорова', 'Кардиолог', 'Москва', 11, 'Клиника «Здоровье», каб. 205', '205', 'ONLINE'],
  ['Дмитрий', 'Новиков', 'Кардиолог', 'Казань', 9, 'Медцентр Казань', '12', 'OFFLINE'],
  ['Ольга', 'Морозова', 'Невролог', 'Москва', 11, 'Невроцентр', '3', 'ONLINE'],
  ['Сергей', 'Лебедев', 'Терапевт', 'Новосибирск', 7, 'Поликлиника №4', '18', 'OFFLINE'],
  ['Анна', 'Фёдорова', 'Офтальмолог', 'Москва', 14, 'Глазная клиника', '7', 'ONLINE'],
  ['Павел', 'Орлов', 'Хирург', 'Санкт-Петербург', 16, 'Городская больница', '402', 'OFFLINE'],
  ['Татьяна', 'Романова', 'Терапевт', 'Екатеринбург', 8, 'УралМед', '21', 'OFFLINE'],
  ['Андрей', 'Зайцев', 'Невролог', 'Казань', 13, 'Невро+', '5', 'ONLINE'],
  ['Елена', 'Белова', 'Офтальмолог', 'Новосибирск', 6, 'ОптикаМед', '2', 'OFFLINE'],
  ['Максим', 'Григорьев', 'Терапевт', 'Москва', 5, 'Клиника «Здоровье», онлайн', 'Онлайн-консультация', 'ONLINE'],
  ['Светлана', 'Комарова', 'Кардиолог', 'Санкт-Петербург', 12, 'Сердечно-сосудистый центр', '310', 'OFFLINE'],
];
...
async function main() {
  ...
  for (const row of DOCTORS) {
    const [firstName, lastName, specName, city, years, clinic, cabinet, presence] =
      row;
    ...
    await client.query(
      `INSERT INTO doctors (
        ...
        "isActive", "visitCount", clinic, cabinet, presence, "lastSeenAt", "userId"
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, true, $8, $9, $10, $11::doctors_presence_enum, NULL, NULL)`,
      [
        id,
        firstName,
        lastName,

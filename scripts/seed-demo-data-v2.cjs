/**
 * То же самое, что seed-demo-data.cjs, но у каждого демо-пациента есть хотя бы одна
 * предстоящая запись (в v1 у пациентов с idx % 4 === 0 их не было).
 * Запуск: node scripts/seed-demo-data-v2.cjs
...
async function seedDemoAppointments(client) {
  ...
  for (let idx = 0; idx < users.length; idx++) {
    ...
    const pastN = 1 + (idx % 4);
    /** Как минимум одна будущая запись на каждого пациента (в v1: upN = idx % 4). */
    const upN = Math.max(1, idx % 4);
  ...
  console.log('demo appointments inserted for', users.length, 'patients (v2: each has upcoming)');
  await syncPatientAndDoctorStats(client);
}
